...
test('test calculate mock', () => {
    ...
    expect(callback.mock.calls[0][1]).toBe('nandang')
    expect(callback.mock.calls[1][0]).toBe(50)
    expect(callback.mock.calls[1][1]).toBe('nandang')

})

test('test calculate mockAndReturn', () => {
    const callback = jest.fn()
...
test('test calculate mock implementation mockAndReturn', () => {
    const callback = jest.fn()
